...
vi.mock('../Header', () => ({
  default: ({ onMenuToggle, onLogoClick, showExitGame, onExitGame }: any) => (
    <header data-testid="header">
      ...
      <button onClick={onLogoClick} data-testid="logo">Logo</button>
      {showExitGame && (
        <button onClick={onExitGame} data-testid="exit-game">Exit Game</button>
      )}
    </header>
  ),
}));
...
describe('Layout Component', () => {
      ...
      <MemoryRouter initialEntries={[initialRoute]}>
        <Routes>
          <Route element={<Layout />}>
            <Route path="/" element={<div data-testid="lobby-content">Lobby</div>} />
            <Route path="/game/:gameSlug" element={<div data-testid="game-content">Game</div>} />
          </Route>
        </Routes>
      </MemoryRouter>
    );
  ...
  describe('Menu Overlay', () => {
    ...
    it('should show Exit Game in menu on game pages', () => {
      ...
      const exitGameLinks = menuLinks.filter(btn => btn.textContent === 'Exit Game');
